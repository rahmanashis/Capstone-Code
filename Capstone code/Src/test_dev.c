...
void display_developer_copyright() {
    ...
    if (GetConsoleScreenBufferInfo(hConsole, &csbi)) {
        ...
        int linesToBottom = consoleHeight - currentY - 3; // Reduced from 6 to 3 lines for more compact display
    ...
    // Print copyright in bottom right corner with more compact format
    set_console_color(COLOR_GRAY);  // Changed from YELLOW to GRAY for less prominence

    // Single line format with smaller visual footprint
    printf("%*s", CONSOLE_WIDTH - 45, "");
    printf("(c) 2025 Devs: H.M.Ashis Rahman | MD.Yeasif Jani Mishad | Redwan Ahmed\n");
